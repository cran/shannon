...
se_f <- function(alpha, beta) {
  ...
  integrand <- function(x, alpha, beta) {
    ((VaRES::dF(x, alpha, beta))*(VaRES::dF(x, alpha, beta, log=TRUE)))
  }
  {
...
#' @export
re_f <- function(alpha, beta, delta) {
  if (any(delta == 1))
...
#' @export
hce_f <- function(alpha, beta, delta) {
 if (any(delta == 1))
    stop(paste("delta cannot take exactly 1"))
  if (any(alpha <= 0))
    stop(paste("alpha must be greater than 0"))
...
#' @export
ae_f <- function(alpha, beta, delta) {
  if (any(delta == 1))
    stop(paste("delta cannot take exactly 1"))
  if (any(alpha <= 0))
    stop(paste("alpha must be greater than 0"))
