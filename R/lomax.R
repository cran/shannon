...
#' @export
re_lom <- function(alpha, beta, delta) {
  if (any(delta == 1))
  ...
  integrand <- function(x, alpha, beta, delta) {
    (VaRES::dlomax(x,alpha, beta))^delta
  }
...
#' @export
hce_lom <- function(alpha, beta, delta) {
  if (any(delta == 1))
    stop(paste("delta cannot take exactly 1"))
  if (any(alpha <= 0))
    stop(paste("alpha must be greater than 0"))
...
#' @export
ae_lom <- function(alpha, beta, delta) {
  if (any(delta == 1))
    stop(paste("delta cannot take exactly 1"))
  if (any(alpha <= 0))
    stop(paste("alpha must be greater than 0"))
